...
import { exportTrace, type OTelResourceSpans, type OTelSpan } from "../src/runtime/trace.ts";
...
test("exportTrace builds valid OTelResourceSpans with root and effect child spans", async () => {
  await withHome(async (home) => {
    ...
    try {
      ...
      assert.ok(trace.resource);
      assert.ok(Array.isArray(trace.resource.attributes));
      const serviceNameAttr = trace.resource.attributes.find((a: { key: string }) => a.key === "service.name");
      assert.equal(serviceNameAttr?.value.stringValue, "hello");

      assert.ok(trace.scopeSpans);
      assert.equal(trace.scopeSpans.length, 1);
      const scopeSpan = trace.scopeSpans[0]!;
...
test("exportTrace throws if journal verification fails", async () => {
  await withHome(async (home) => {
    ...
    // Tamper with journal
    const journal = openJournal(paths.journal);
    // Append a fake broken event with invalid hash to break chain
    // (openJournal verifies chain integrity)
    journal.close();

    // Now test verifyChain failure on tampered journal
...
test("CLI run --trace emits valid OTel JSON to stdout", async () => {
  await withHome(async (home) => {
    const capsule = await packFixture(home);
    ...
    const parsed = JSON.parse(stdout) as OTelResourceSpans;
    assert.ok(parsed.resource);
    assert.ok(parsed.scopeSpans);
    assert.equal(parsed.scopeSpans[0]!.spans[0]!.name, "execute_tool greet");
    assert.equal(parsed.scopeSpans[0]!.spans[0]!.status.code, 1);
  });
});
...
test("exportTrace falls back to sha256 traceId on malformed traceparent and uses defaults when name/version omitted", async () => {
  await withHome(async (home) => {
    ...
    try {
      ...
      assert.equal(trace.scopeSpans[0]!.scope.name, "agent-capsule");
    } finally {
      journal.close();
    }
  });
...
test("CLI run --trace --json outputs single line JSON", async () => {
  await withHome(async (home) => {
    const capsule = await packFixture(home);
