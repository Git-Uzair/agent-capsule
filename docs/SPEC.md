...
A conforming runtime MUST enforce the following semantic invariants for `capsule.json`. Invariants 1 and 3–5, plus invariant 6's rejection of `..` path segments, are enforced while parsing the manifest (`parseManifest`); invariant 6's requirement that the referenced paths exist is enforced at pack time by `capsule pack` (see item 6), because a manifest can be parsed without the container it points at; invariant 2 is enforced when the tool catalog is built, because it also covers the runtime's built-in names, which are not part of the manifest:

1. **Reserved Tool Names:** Tool names MUST NOT start with `capsule_`. The prefix `capsule_` is reserved exclusively for runtime built-in tools (`capsule_info`, `capsule_runs`, `capsule_replay`). Manifest parsing enforces this rule with `E_CONTENT: reserved tool name: <name>` (`src/format/manifest.ts:131`), and rejects exact duplicate tool names in the same pass with `E_MANIFEST: duplicate tool name: <name>` (`src/format/manifest.ts:134`) — a reserved name is a content violation, a duplicate is a malformed manifest.
...
6. **Path Traversal Immunity:** All relative path references (`runtime.entry`, `resources[].path`, `ui.app.path`, `ui.local.path`) MUST NOT contain `..` path segments and MUST point to existing container entries. The two halves are enforced at different times: `parseManifest` rejects a `..` segment in any of those paths (`E_MANIFEST: path must not contain '..': <path>`, `src/format/manifest.ts:124`), while existence is checked when the container is built — `capsule pack` fails with `E_MANIFEST: entry file not found: <path>` for any referenced path that is not among the packed entries (`src/format/capsule.ts:81`), naming the missing path whether it is the runtime entry, a resource, or a UI document.
...
Every row below states the unit its limit is counted in and how the limit is enforced. Unless a row says otherwise, exceeding a limit is a refusal: the effect throws and no result is produced. `E_USAGE` marks a malformed or oversized request from the guest, `E_POLICY` marks something the guest is not permitted to do at all, and `E_TIMEOUT` marks a deadline. `log.write` is the single exception in the vocabulary — it truncates instead of refusing. Independently of these limits, every effect is refused with `E_POLICY` unless the calling tool declared it in `tools[].effects` (§2).

| Operation | Parameters | Return Value | Limits & Security Policy |
| :--- | :--- | :--- | :--- |
| `clock.now` | `{}` | ISO-8601 UTC timestamp string | No limit. Derived from the host clock in record mode and from the journal in replay (§6.4). |
| `random.bytes` | `{ n: integer }` | Hexadecimal string of `2n` characters | `n` is a byte count: `1 <= n <= 64`. A non-integer, out-of-range or missing `n` is refused with `E_USAGE` (`src/runtime/effects.ts:99`). |
| `kv.get` | `{ key: string }` | `string` or `null` | Key `<= 256` characters (UTF-16 code units); a longer key is refused with `E_USAGE`. Requires `capabilities.kv = true`, otherwise `E_POLICY`. |
| `kv.set` | `{ key: string, value: string }` | `true` | Key `<= 256` characters (UTF-16 code units), value `<= 64` KiB (65,536 UTF-8 bytes), table `<= 10,000` rows — a key that already exists may still be updated once the table is full, only a new key is refused. Each of the three is a refusal with `E_USAGE` (`src/runtime/state.ts:7`). Requires `capabilities.kv = true`, otherwise `E_POLICY`. |
| `sql.query` | `{ sql: string, params?: array }` | Array of row objects | Read-only connection. Same leading-keyword gate as `sql.exec` (refusal with `E_POLICY`). Result `<= 1,000` rows and `<= 1` MiB (1,048,576 bytes, text counted as UTF-8 and blobs by their own length), both measured while rows arrive and refused with `E_USAGE` on the row that crosses the budget. Requires `capabilities.sql = true`, otherwise `E_POLICY`. |
| `sql.exec` | `{ sql: string, params?: array }` | `{ changes: integer }` | Read-write connection. Both SQL ports strip leading whitespace, comments and empty statements, then reject the statement with `E_POLICY` if its leading keyword is `ATTACH`, `DETACH`, `PRAGMA`, or `VACUUM` (`src/runtime/state.ts:14`). Only the first statement of the string is compiled and run, so a second appended statement never executes; no row or byte limit applies to a write. Requires `capabilities.sql = true`, otherwise `E_POLICY`. |
| `log.write` | `{ message: string }` | `true` | Message length `<= 2048` characters (UTF-16 code units), counted after sanitisation. The message is sanitised (NFKC-normalised, ANSI escapes, control, zero-width and bidi characters stripped) and silently truncated with a ` …[truncated]` suffix, then written to host `stderr` with a trailing newline; the call returns `true` and is never refused for length. A `message` that is not a string is refused with `E_USAGE` (`src/runtime/effects.ts:122`). |
| `net.fetch` | `{ url: string, init?: object }` | `{ status, statusText, headers, body }` | HTTPS only (or HTTP on loopback when `capabilities.net.allow_localhost = true`). Host allowlist validation, no credentials in the URL, port restricted to 80, 443 or 1024–65535, and SSRF protection (rejects RFC 1918 / carrier-NAT / link-local / cloud metadata / loopback / reserved IPs) — each of these, and a redirect into any of them, is a refusal with `E_POLICY`. `GET` and `POST` only, `<= 16` request headers of `<= 1,024` UTF-8 bytes each, request body `<= 1` MiB (1,048,576 UTF-8 bytes), response body `<= 4` MiB (4,194,304 bytes, refused from the declared `content-length` or while streaming), max 5 redirects — each of these is a refusal with `E_USAGE`. One 5,000 ms deadline covers the whole exchange, redirects included, and expiry is `E_TIMEOUT`. |
...
1. **Record Mode (`mode = "record"`):** Standard tool invocation. Non-deterministic effects (`clock.now`, `random.bytes`, `sql.*`, `kv.*`, `net.fetch`, `log.write`) are executed by host dispatchers, and each event with its parameters and results is appended to the hash-chained SQLite journal. `pack.write` is absent from this list because no host port is wired in the shipped runtime (§5). It is still dispatchable, so the ordinary journalling order applies: `effect.requested` is appended before the port is called, and it is the only row the attempt leaves — the dispatch then raises `E_USAGE: pack.write is not available in this runtime`, so no `effect.completed` follows. A recorded `pack.write` therefore always appears as a request with no completion, which is the same shape any failed effect leaves behind. When `CAPSULE_JOURNAL_ARGS=1` is set, invocation arguments are recorded in the journal to enable replay.
